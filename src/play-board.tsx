...
function PlayerItem(props: {
  ...
  return (
    <div className={`player-item${player.isDead ? " dead" : ""}`}>
      {player.name}
      {props.president ? " (P)" : ""}
      {props.chancellor ? " (C)" : ""}
      {player.isConfirmedNotHitler && (
        <div className="not-hitler">Not Hitler!</div>
      )}
      <div
        className={`vote${vote.show ? "" : " hidden"} ${
          vote.vote ? "ja" : "nein"
        }`}
      >
        {vote.vote ? "JA!" : "NEIN!"}
...
export function PlayBoard(props: PlayBoardProps) {
  ...
  const revealLib =
    props.state.type == "cardReveal" && props.state.card == "Liberal"
  const revealFas =
    props.state.type == "cardReveal" && props.state.card == "Fascist"

  const voteHasResult =
    props.state.type == "election" && props.state.voteResult != null
  ...
  useSound(
    electChancellorSound,
    props.state.type == "election" && props.state.chancellorElect == undefined
  )
  useSound(
    executePlayerSound,
    props.state.type == "executiveAction" && props.state.action == "execution"
  )
  useSound(
    playerDeathSound,
    props.state.type == "executiveAction" &&
      props.state.action == "execution" &&
      props.state.playerChosen != null
  )
  useSound(
    investigateSound,
    props.state.type == "executiveAction" && props.state.action == "investigate"
  )
  useSound(
    policyPeekSound,
    props.state.type == "executiveAction" && props.state.action == "policyPeak"
  )
  useSound(secretRoleSound, props.state.type == "nightRound")

  let electionTracker = props.electionTracker
  if (
    props.state.type == "election" &&
    showResult &&
  ...
  if (
    props.state.type == "legislativeSession" &&
  ...
  return (
    <div className="play-board">
      {gameStarted && (
        <>
          <PolicyTracker
            screen={screen}
            party="Liberal"
            numCards={props.numLiberalCards}
            reveal={revealLib}
          ...
          <PolicyTracker
            screen={screen}
            party="Fascist"
            numCards={props.numFascistCards}
            reveal={revealFas}
            numPlayers={numPlayers}
          />
        </>
      ...
      <div className="util">
        ...
        <div>
          <ElectionTracker tracker={electionTracker} deck={props.drawPile} />
        </div>
      </div>
      <div className="modal-wrap">
        {modalTransitions.map(({ item, key, props: style }) => {
          ...
          return modal ? (
            <animated.div key={key} className={cl} style={style}>
              {modal}
            </animated.div>
          ) : null
        })}
      </div>
      <h1 className={`chaos${showChaos && !hideChaos ? " show" : ""}`}>
        Chaos!
      </h1>
    </div>
  )
